...
class ALBEF(nn.Module):
    ...
    def forward(self, image, text, alpha=0, mask_query_interp = None, gt_mask_indicator = None):
        with torch.no_grad():
        ...
        if mask_query_interp is not None and gt_mask_indicator.sum().item() > 0 and self.add_gcam:
            
            gt_mask_indicator = gt_mask_indicator.squeeze()
            mask_query_interp = mask_query_interp[gt_mask_indicator>0]
            ...
            in_box_values = gradcam*mask_query_interp
            out_box_values = gradcam*(1-mask_query_interp)
            avg_in_box_values = torch.sum(in_box_values)/torch.sum(mask_query_interp)
            avg_out_box_values = torch.sum(out_box_values)/torch.sum(1-mask_query_interp)
            # margin = 0.1 
            loss_gcam_masked1 = torch.max(torch.tensor([0.]).to(image.device), avg_out_box_values-avg_in_box_values+0.1)

            max_in_box_values = torch.max(in_box_values.view(in_box_values.size(0),-1),dim=1)[0]
            max_out_box_values = torch.max(out_box_values.view(out_box_values.size(0),-1),dim=1)[0]
            # margin = 0.5 
            loss_gcam_masked2 = torch.max(torch.tensor([0.]).to(image.device), max_out_box_values-max_in_box_values+0.5).mean()
        ...
        if mask_query_interp is not None and gt_mask_indicator.sum().item() > 0 and self.add_gcam:
            return loss_mlm, loss_ita, loss_itm, loss_gcam_masked1, loss_gcam_masked2
        else:
